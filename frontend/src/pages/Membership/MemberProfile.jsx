    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-8 pb-16">
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex items-start gap-6">
                  {member.logo ? (
                    <div className="w-24 h-24 rounded-2xl overflow-hidden border-2 border-slate-200 shadow-lg flex-shrink-0 flex items-center justify-center bg-white">
                      <img src={urlFor(member.logo).url()} alt={`${member.company} logo`} className="max-w-full max-h-full object-cover" />
                    </div>
                  ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-800 border border-green-200">{member.membershipType}</span>
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
              <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center">
                <User className="mr-3 text-green-700" size={28} /> Key Representative
              </h2>
              <div className="flex items-center gap-6">
                {member.keyRepresentative?.photo ? (
                  <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-slate-200 flex-shrink-0">
                    <img src={urlFor(member.keyRepresentative.photo).width(200).height(200).url()} alt={member.keyRepresentative.name} className="w-full h-full object-cover" />
                  </div>
                ) : (
            ...
            {member.overview && (
              <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
                <h2 className="text-2xl font-bold text-slate-900 mb-4 flex items-center">
                  <Briefcase className="mr-3 text-green-700" size={28} /> Company Overview
                </h2>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 text-center">
              ...
              <p className="text-xs text-slate-500 mt-4 leading-relaxed">
                Scan this code to view this business profile on your mobile device.
              </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-green-900 to-green-700 text-white rounded-2xl shadow-xl p-8">
              ...
              <p className="text-green-100 text-sm mb-6">Ready to connect? Reach out to us below.</p>
              <div className="space-y-4">
                <a href={`mailto:${member.email}`} className="block w-full bg-white text-green-900 px-6 py-3 rounded-lg font-semibold hover:bg-green-50 transition-all text-center shadow-lg">
                  <Mail className="inline mr-2" size={18} /> Contact via Email
                </a>
                {member.phone && (
                  <a href={`tel:${member.phone}`} className="block w-full bg-green-800 text-white px-6 py-3 rounded-lg font-semibold hover:bg-green-900 transition-all text-center border-2 border-white/20">
                ...
                {member.phone && (
                  <div className="flex items-start">
                    ...
                    <div className="ml-3">
                      ...
                      <a href={`tel:${member.phone}`} className="text-slate-700 hover:text-green-700 font-medium">{member.phone}</a>
                    </div>
                  </div>
                )}
                <div className="flex items-start">
                  ...
                  <div className="ml-3">
                    ...
                    <a href={`mailto:${member.email}`} className="text-slate-700 hover:text-green-700 font-medium break-all">{member.email}</a>
                  </div>
                </div>
                  ...
                  <div className="pt-4 border-t border-slate-100">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {member.socialMedia.facebook && (
                        <a href={member.socialMedia.facebook} target="_blank" rel="noopener noreferrer" className="p-2 bg-blue-50 hover:bg-blue-600 text-blue-600 hover:text-white rounded-lg transition-all"><Facebook size={18} /></a>
                      )}
                      {member.socialMedia.linkedin && (
                        <a href={member.socialMedia.linkedin} target="_blank" rel="noopener noreferrer" className="p-2 bg-blue-50 hover:bg-blue-700 text-blue-700 hover:text-white rounded-lg transition-all"><Linkedin size={18} /></a>
                      )}
                      {member.socialMedia.instagram && (
                        <a href={member.socialMedia.instagram} target="_blank" rel="noopener noreferrer" className="p-2 bg-pink-50 hover:bg-pink-600 text-pink-600 hover:text-white rounded-lg transition-all"><Instagram size={18} /></a>
                      )}
                      {member.socialMedia.twitter && (
                        <a href={member.socialMedia.twitter} target="_blank" rel="noopener noreferrer" className="p-2 bg-slate-50 hover:bg-slate-900 text-slate-700 hover:text-white rounded-lg transition-all"><FaXTwitter size={18} /></a>
                      )}
                    </div>
                  </div>
                )}
                ...
                {member.website && (
                  <div className="flex items-start">
                    ...
                    <div className="ml-3">
                      ...
                      <a href={member.website.startsWith("http") ? member.website : `https://${member.website}`} target="_blank" rel="noopener noreferrer" className="text-slate-700 hover:text-green-700 font-medium break-all inline-flex items-center">
                        {member.website.replace(/^https?:\/\//, "")} <ExternalLink size={14} className="ml-1" />
                      </a>
                    </div>
                  </div>
                )}
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6">
              <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center">
                <MapPin className="mr-2 text-green-700" size={20} /> Our Location
              </h3>
              <p className="text-slate-700 leading-relaxed mb-4">{member.location}</p>
              <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(member.location)}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-green-700 hover:text-green-900 font-semibold text-sm">
            ...
            {member.businessHours && (
              <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6">
                <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center">
                  <Clock className="mr-2 text-green-700" size={20} /> Business Hours
                </h3>
                  ...
                  {days.map((day) =>
                    member.businessHours?.[day] ? (
                      <div key={day} className="flex justify-between items-center py-2 border-b border-slate-100 last:border-0">
                        ...
                        <span className={`text-sm font-semibold ${member.businessHours[day] === "Closed" ? "text-slate-400" : "text-slate-700"}`}>{member.businessHours[day]}</span>
                      </div>
                    ) : null
                  )}
                </div>
        ...
        <div className="mt-12 bg-gradient-to-r from-green-900 to-green-700 rounded-2xl p-8 text-center text-white shadow-2xl">
          ...
          <Link to="/join" className="inline-block bg-white text-green-900 px-8 py-3 rounded-lg font-bold hover:bg-green-50 transition-all shadow-lg">Become a Member</Link>
        </div>
      </div>
